...
aliases:
- /performant-gemv-kernel
permalink: /performant-gemv-kernel
badges: false
toc: true
categories:
- LLMs
...
date: '2026-09-10'
description: A worklog on optimizing a CUDA GEMV kernel—from arithmetic intensity and uncoalesced loads to a warp-per-row implementation that makes every byte count.
hide: false
output-file: performant_gemv_kernel.html
search_exclude: false

---

Code: https://github.com/athleticcoder21/kernels
...
![](./assets/gemv_kernel/image1-v2.png){fig-alt="A hand-drawn GEMV overview showing an M by N matrix A multiplied by a length-N vector x to produce a length-M vector y."}
...
![](./assets/gemv_kernel/naive_gemv_memory_layout_v2.svg){fig-alt="A three by four matrix is flattened into a single row of twelve memory locations. The highlighted element A[2,2] maps to offset 2 times 4 plus 2, which is linear index 10." width=100%}
...
![](./assets/gemv_kernel/naive_gemv_execution.svg){fig-alt="Naive GEMV access pattern. Each thread traverses one contiguous row of flattened A, uses the shared vector x, and writes one element of y." width=100%}
...
![](./assets/gemv_kernel/coalesced_gemv_warp_access_v2.svg){fig-alt="A coalesced GEMV access pattern in which one warp owns one matrix row. On the first pass threads zero through thirty-one access adjacent columns zero through thirty-one. On the second pass the same threads access adjacent columns thirty-two through sixty-three. Each pass uses four contiguous 32-byte sectors, transferring 128 useful bytes in 128 bytes of memory traffic." width=100%}
